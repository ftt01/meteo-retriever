...
input_parser.add_argument('-y','--year', type=str, default='2012')
...
def execute(params):
    ...
    moved_path = f"{input_path}todo/"

    # ## delete old grib
    # try:
    #     rm_cmd = f"rm -rf {moved_path}*.grib"
    #     rm_process = subprocess.Popen(rm_cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    #     stdout, stderr = rm_process.communicate()
    #     if ('Error' in str(stderr) or 'Warning' in str(stderr)):
    #         print(stderr)
    # except Exception as e:
    #     print(f"Not necessary to delete: {e}")

    ## copy to the todo directory
    mkNestedDir( moved_path )
    cmd = f"cp -rf {input_path}{filename} {moved_path}{filename}"

    copy_process = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
